...
void TestMCLSerializeG1()
{
    for (int i = 0; i < 8; i++)
    {
        array<uint8_t, 48> compressedBytes = hexToBytes<48>(compressed_mcl_g1_le[i]);
        auto g1Option = g1::fromCompressedMCLBytesLE(compressedBytes);
        if(!g1Option.has_value())
        {
            throw invalid_argument("error deserializing g1 from mcl format");
        }

        g1 a = g1Option.value();
        array<uint8_t, 96> affineBytes = a.toAffineBytesBE();
        array<uint8_t, 96> expectedAffineBytes = hexToBytes<96>(decompressed_mcl_g1_be[i]);
        if (!arrayEquals(affineBytes, expectedAffineBytes))
        {
            throw invalid_argument("incorrect deserialization of g1 with mcl format");
        }

        array<uint8_t, 48> serializedCompressedBytes = a.toCompressedMCLBytesLE();
        if (!arrayEquals(serializedCompressedBytes, compressedBytes))
        {
            throw invalid_argument("incorrect serialization of g1 with mcl format");
        }
    }
    printf("g1 - done\n");
}

void TestMCLSerializeG2()
{
    for (int i = 0; i < 2; i++)
    {
        array<uint8_t, 96> compressedBytes = hexToBytes<96>(compressed_mcl_g2_le[i]);
        auto g2Option = g2::fromCompressedMCLBytesLE(compressedBytes);
        if(!g2Option.has_value())
        {
            throw invalid_argument("error deserializing g2 from mcl format");
        }

        g2 a = g2Option.value();
        array<uint8_t, 192> affineBytes = a.toAffineBytesBE();
        array<uint8_t, 192> expectedAffineBytes = hexToBytes<192>(decompressed_mcl_g2_be[i]);
        if (!arrayEquals(affineBytes, expectedAffineBytes))
        {
            throw invalid_argument("incorrect deserialization of g2 with mcl format");
        }

        array<uint8_t, 96> serializedCompressedBytes = a.toCompressedMCLBytesLE();
        if (!arrayEquals(serializedCompressedBytes, compressedBytes))
        {
            throw invalid_argument("incorrect serialization of g2 with mcl format");
        }
    }
    printf("g2 - done\n");
}
